...
function called enumarate(). This will rertun tuple(tuple is a read only list can not modify) for each iteration. '''

for letter in enumerate(letters):
    print(letter)

''' Now if we print letter then you get tuple, first item of the tuple is index and second one is item of index. 
so now if you print letter by index 0 and 1 you get index and letters. '''

for letter in enumerate(letters):
    print(letter[0], letter[1])

''' Now we can unpack this tuple as we unpack list. Here enumerate() returns tuple for each itaration like 
